...
%global npm_bare      python-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-python-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Python configuration providing standardized Ruff, Black, Flake8, and MyPy rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/python-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/python-config is a shareable configuration preset with no runtime
...
install -pm 0644 pyproject.toml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 ruff.toml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 .flake8 %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
